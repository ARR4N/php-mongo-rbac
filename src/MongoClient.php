...
class MongoClient extends \MongoClient {
	 ...
	 * @param $userId		callable				a function callback to return the current integer user ID; caching does not occur and is the responsiblity of the developer should it be warranted
	 */
	public function __construct(\PhpRbac\Rbac $rbac, callable $userId){
		$args = func_get_args();
	 ...
	 *
	 * @param $action 	string		one of a set of predefined actions; these can be seen in the switch statement
	...
	public function rbacCheck($action, $obj, array $metaData = null){
		switch($action){
			case "db_construct":
				$path = "/mongo/db/{$obj->getName()}";
				break;
			case "collection_construct":
				$path = "/mongo/collections/{$obj->db->getName()}/".(str_replace(".", "/", $obj->getName()));
				break;
			default:
		...
		if(is_null($pathId)){
			...
			if($this->_rbac->check($pathId, $userId) !== true){
